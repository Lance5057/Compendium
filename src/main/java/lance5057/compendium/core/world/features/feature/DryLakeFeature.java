...
public class DryLakeFeature extends Feature<BlockStateFeatureConfig> {
    private static final BlockState AIR = Blocks.CAVE_AIR.getDefaultState();

    public DryLakeFeature(Codec<BlockStateFeatureConfig> p_i231968_1_) {
	super(p_i231968_1_);
    }
		...
		for (int k1 = 0; k1 < 16; ++k1) {
			...
			for (int k = 0; k < 8; ++k) {
				...
				if (k < 4 && !material.isSolid()
					&& reader.getBlockState(pos.add(k1, k, l2)) != config.state) {
				    return false;
				}
			    }
			}
		...
		for (int j2 = 0; j2 < 16; ++j2) {
			...
			for (int k4 = 0; k4 < 8; ++k4) {
			    ...
			    if (flag1 && (k4 < 4 || rand.nextInt(2) != 0)
				    && reader.getBlockState(pos.add(j2, k4, k3)).getMaterial().isSolid()) {
				reader.setBlockState(pos.add(j2, k4, k3),
					CompendiumBlocks.DRYLAKEBED.get().getDefaultState(), 2);
			    }
			}
		    }
		}
