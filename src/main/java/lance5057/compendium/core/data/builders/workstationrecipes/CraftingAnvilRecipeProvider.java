...
public class CraftingAnvilRecipeProvider extends RecipeProvider {
	...
	protected void registerRecipes(@Nonnull Consumer<IFinishedRecipe> consumer) {
		AnvilShapedRecipeBuilder.shapedRecipe(Items.IRON_SWORD, 8).key('#', Items.STICK).key('X', Items.IRON_INGOT).patternLine("X").patternLine("X").patternLine("#").addCriterion("has_iron_ingot", hasItem(Items.IRON_INGOT)).build(consumer, "anvil/iron_sword");
	}
	...
	public static class AnvilResult implements IFinishedRecipe {
		...
		public AnvilResult(ResourceLocation idIn, Item resultIn, int strikesIn, int countIn, String groupIn, List<String> patternIn, Map<Character, Ingredient> keyIn, Advancement.Builder advancementBuilderIn, ResourceLocation advancementIdIn) {
			this.id = idIn;
