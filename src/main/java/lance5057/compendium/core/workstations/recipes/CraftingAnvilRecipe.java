...
public class CraftingAnvilRecipe implements IShapedRecipe<RecipeWrapper> {
	...
	public CraftingAnvilRecipe(ResourceLocation idIn, String groupIn, int recipeWidthIn, int recipeHeightIn, NonNullList<Ingredient> recipeItemsIn, ItemStack recipeOutputIn, int strikes) {
		this.id = idIn;
		this.group = groupIn;
	...
	public static class Serializer extends net.minecraftforge.registries.ForgeRegistryEntry<IRecipeSerializer<?>> implements IRecipeSerializer<CraftingAnvilRecipe> {
		...
		public CraftingAnvilRecipe read(ResourceLocation recipeId, JsonObject json) {
			...
			String[] astring = CraftingAnvilRecipe.shrink(CraftingAnvilRecipe.patternFromJson(JSONUtils.getJsonArray(json, "pattern")));
			int i = astring[0].length();
			int j = astring.length;
			NonNullList<Ingredient> nonnulllist = CraftingAnvilRecipe.deserializeIngredients(astring, map, i, j);
			ItemStack itemstack = ShapedRecipe.deserializeItem(JSONUtils.getJsonObject(json, "result"));
			int strikes = json.get("strikes").getAsInt();
