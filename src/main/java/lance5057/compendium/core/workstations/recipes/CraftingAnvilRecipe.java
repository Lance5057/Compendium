...
public class CraftingAnvilRecipe implements IShapedRecipe<WorkstationRecipeWrapper> {
    ...
    private final int toolListLength;
	...
	this.toolListLength = toolLength;
	this.id = idIn;
	this.group = groupIn;
    ...
    {
	return this.schematic.equals(inv.getSchematic(), true);
    }
...
//	    }
	...
	public CraftingAnvilRecipe read(ResourceLocation recipeId, JsonObject json) {
	    ...
	    return new CraftingAnvilRecipe(recipeId, s, i, j, nonnulllist, nonnulllistTools.size(), nonnulllistTools,
		    itemstackSchem, itemstack);
	}
	...
	public CraftingAnvilRecipe read(ResourceLocation recipeId, PacketBuffer buffer) {
	    ...
	    return new CraftingAnvilRecipe(recipeId, s, i, j, nonnulllist, h, nonnulllistTool, itemstackSchem,
		    itemstack);
	}
	...
	public void write(PacketBuffer buffer, CraftingAnvilRecipe recipe) {
	    ...
	    buffer.writeVarInt(recipe.toolListLength);

	    for (RecipeItemUse riu : recipe.recipeTools)
		RecipeItemUse.write(riu, buffer);

	    buffer.writeItemStack(recipe.schematic);
