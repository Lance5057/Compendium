...
public class CompendiumTier extends SimpleTier {
	String vanilla;
	...
	public CompendiumTier(TagKey<Block> incorrectBlocksForDrops, int uses, float speed, float attackDamageBonus,
	...
	public CompendiumTier(Tier tier) {
	...
	public CompendiumTier(String tier) {
		this(Tiers.valueOf(tier));
		vanilla = tier;
	}

	public static CompendiumTier deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
			throws JsonParseException {
		JsonObject j = json.getAsJsonObject();

		if (j.has("vanillaTier")) {
			return new CompendiumTier(j.get("vanillaTier").getAsString());
		}
			...
			return new CompendiumTier(BlockTags.create(ResourceLocation.parse(useTag)), uses, speed, damage,
					enchantmentValue, repair);
		}

	}

	public static JsonElement serialize(CompendiumTier src, Type typeOfSrc, JsonSerializationContext context) {
