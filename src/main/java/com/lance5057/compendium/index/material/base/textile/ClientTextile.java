		...
		if (mb instanceof MaterialTextile mt) {
			...
			for (String b : StyleData.TABLE_CLOTH.getTypes()) {
				...
				if (b.contains("angled")) {
					...
					event.getModels().put(m, CompendiumClient.basicModelManyTexture(event, loc, m,
					...
					event.getModels().put(new ModelResourceLocation(modelLoc_inv, ""),
							CompendiumClient.basicModelManyTexture(event, loc_inv,
									...
									Pair.of("0", texture), Pair.of("1", Compendium
											.modLoc("block/material/textile/" + mb.name + "/diagonal_half"))));
				} else {
					event.getModels().put(m, CompendiumClient.basicModelAllTexture(event, texture, loc, m,
					...
					for (BedPart part : BedPart.values()) {
						...
						for (int occupied = 0; occupied < 2; occupied++) {
							...
							if (b.equals("llama")) {
								...
								mmPart.add(s -> s.getValue(FancyBedBlock.OCCUPIED) == bo, CompendiumClient.doBed(event,
										loc, m, Pair.of("0", Compendium.modLoc(mb.blockFolder() + "woolly_glazed"))));
							} else {
