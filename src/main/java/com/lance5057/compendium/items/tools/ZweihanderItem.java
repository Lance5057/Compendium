...
public class ZweihanderItem extends SwordItem {
	...
	public ZweihanderItem(Tier tier, int attackDamageIn, float attackSpeedIn, Item.Properties builderIn) {
			...
			if (playerIn.getItemInHand(InteractionHand.OFF_HAND).isEmpty())
				if (!playerIn.hasEffect(MobEffects.WEAKNESS)) {
					...
					for (Entity e : ents) {
						...
						worldIn.addParticle(ParticleTypes.SWEEP_ATTACK, e.getX(), e.getY() + 1, e.getZ(), 0, 0, 0);
					}
