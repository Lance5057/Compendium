...
//	}
	...
	private static final Codec<WorkbenchRecipe> CODEC = RecordCodecBuilder.create(inst -> inst.group(
			ExtraCodecs.strictOptionalField(Codec.STRING, "group", "").forGetter(WorkbenchRecipe::getGroup),
			ExtraCodecs.strictOptionalField(Codec.INT, "width", 3).forGetter(r -> r.getRecipeWidth()),
			ExtraCodecs.strictOptionalField(Codec.INT, "height", 3).forGetter(r -> r.getRecipeHeight()),
			Ingredient.LIST_CODEC_NONEMPTY.fieldOf("ingredients").xmap(ingredients -> {
				NonNullList<Ingredient> nonNullList = NonNullList.create();
				nonNullList.addAll(ingredients);
				return nonNullList;
			}, ingredients -> ingredients).forGetter(WorkbenchRecipe::getIngredients),
